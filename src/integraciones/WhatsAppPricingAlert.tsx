...
// start being charged per message.
//
// This is OPTION 1 (review modal). See WhatsAppConsentAlert for OPTION 2.
...
export default function WhatsAppPricingAlert() {
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', borderRadius: 10, background: WARN_BG, marginBottom: 16 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <span style={{ fontSize: 14, fontWeight: 700, color: WARN_DARK }}>Nueva política de precios de Meta</span>
        ...
        <button
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 100, border: 'none', background: WARN_DARK, color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', flexShrink: 0, whiteSpace: 'nowrap' }}
        ...
        {step === 'info' ? (
          <>
            ...
            <div className="wa-modal-scroll" style={{ overflowY: 'auto', padding: '24px 28px 28px' }}>
              <Eyebrow color={color.grey500}>Actualización de precios · octubre 2026</Eyebrow>

              <h2 style={{ margin: '12px 0 0', fontSize: 20, fontWeight: 800, lineHeight: 1.25, letterSpacing: '-0.4px', color: color.grey900, maxWidth: '92%' }}>
                Nueva política de precios de Meta
              </h2>
              <p style={{ margin: '10px 0 0', fontSize: 14, lineHeight: 1.65, color: color.grey700 }}>
              ...
              <a href="https://business.whatsapp.com/products/platform-pricing" target="_blank" rel="noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, marginTop: 12, fontSize: 14, fontWeight: 700, color: INFO, textDecoration: 'none' }}>
                Ver el detalle de precios de Meta
                <Icon name="open_in_new" size={15} color={INFO} />
              </a>

              {/* Strategy */}
              <h3 style={{ margin: '26px 0 0', fontSize: 16, fontWeight: 800, color: color.grey900 }}>Cómo evitar ese costo</h3>
              <p style={{ margin: '8px 0 0', fontSize: 14, lineHeight: 1.65, color: color.grey700 }}>
            ...
            {/* Footer */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '16px 24px', borderTop: `1px solid ${color.grey200}` }}>
              <StepHint current={1} />
          ...
          <>
            {/* ── Step 2 — choose lines ─────────────────────────────── */}
            <div className="wa-modal-scroll" style={{ overflowY: 'auto', padding: '24px 28px 28px' }}>
              <button onClick={() => setStep('info')} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 14, fontWeight: 700, color: color.grey600 }}>
                <Icon name="arrow_back" size={16} color={color.grey600} />
                Volver
              </button>

              <h2 style={{ margin: '14px 0 0', fontSize: 20, fontWeight: 800, lineHeight: 1.25, letterSpacing: '-0.4px', color: color.grey900 }}>
                Elegí en qué líneas aplicarlo
              </h2>
              <p style={{ margin: '8px 0 0', fontSize: 14, lineHeight: 1.6, color: color.grey700 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, margin: '20px 0 8px' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'baseline', gap: 12 }}>
                  <button onClick={() => onSetAll(!allOn)} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 14, fontWeight: 700, color: INFO }}>
                    {allOn ? 'Quitar todas' : 'Activar todas'}
                  </button>
                  <span style={{ fontSize: 12, color: color.grey500 }}>{count} de {ACCOUNTS.length}</span>
                </div>
              </div>
                ...
                {ACCOUNTS.map(acc => {
                  ...
                  return (
                    <div
                      ...
                      role="switch"
                      aria-checked={on}
                      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', borderRadius: 14, cursor: 'pointer', border: `1.5px solid ${on ? INFO : color.grey200}`, background: on ? '#F5F7FF' : '#fff', transition: 'border-color .15s, background .15s' }}
                    >
                      <IconBox
                      ...
                      />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: color.grey900 }}>{acc.profileName}</div>
                        <div style={{ fontSize: 12, color: color.grey500, marginTop: 1 }}>{acc.phone}</div>
                      </div>
...
function Pill({ children, bg, color: c }: { children: ReactNode; bg: string; color: string }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 100, background: bg, color: c, fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap', flexShrink: 0 }}>
      {children}
    </span>
  )
}
...
function Eyebrow({ children, color: c }: { children: ReactNode; color: string }) {
  return <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: c }}>{children}</span>
}

// Highlighted keyword, like the yellow marks in the ClickUp email.
function Mark({ children }: { children: ReactNode }) {
  return <mark style={{ background: '#FFF1A8', color: 'inherit', padding: '0 4px', borderRadius: 3, fontWeight: 600 }}>{children}</mark>
}
...
const ghostBtn: CSSProperties = { padding: '12px 20px', borderRadius: 100, border: `1px solid ${color.grey300}`, background: '#fff', color: color.grey700, fontSize: 14, fontWeight: 600, cursor: 'pointer' }
...
function strip(bg: string, border: string): CSSProperties {
  return { display: 'flex', alignItems: 'center', gap: 12, padding: '8px 12px', borderRadius: 10, background: bg, border: `1px solid ${border}`, marginBottom: 16 }
}
function linkBtn(c: string): CSSProperties {
  return { marginLeft: 'auto', background: 'none', border: 'none', color: c, fontSize: 14, fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }
}
